...
# TODO: Replace <FILL IN> with appropriate code
amazonRecToToken = amazonSmall.map(lambda item: (item[0],tokenize(item[1])))
googleRecToToken = googleSmall.map(lambda item: (item[0],tokenize(item[1])))
...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda item: len(item[1])).sum()
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.takeOrdered(1, key = lambda item: -len(item[1]))
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.map(lambda item: list(set(item[1])))
...
def dotprod(a, b):
    ...
    """
    return sum([a[term]*b[term] for term in a.viewkeys() & b.viewkeys()])
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda item: (item[1] + ' ' + item[0], item[2]))
trueDupsRDD = (sims
               .join(goldStandard))
trueDupsCount = trueDupsRDD.count()

avgSimDups = trueDupsRDD.map(lambda item: item[1][0]).sum()/float(trueDupsCount)

nonDupsRDD = (sims
              .subtractByKey(goldStandard))
avgSimNon = nonDupsRDD.map(lambda item: item[1]).sum()/float(nonDupsRDD.count())
...
amazonWeightsRDD = amazonFullRecToToken.map(lambda item: (item[0], tfidf(item[1],idfsFullWeights)))
googleWeightsRDD = googleFullRecToToken.map(lambda item: (item[0], tfidf(item[1],idfsFullWeights)))
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.map(lambda item: (item[0], norm(item[1])))
amazonNormsBroadcast = sc.broadcast(amazonNorms.collectAsMap())
googleNorms = googleWeightsRDD.map(lambda item: (item[0], norm(item[1])))
...
def fastCosineSimilarity(record):
    ...
    s = sum((amazonWeightsBroadcast.value[amazonRec][token] * googleWeightsBroadcast.value[googleRec][token] 
             for token in tokens))
    value = s/float(amazonNormsBroadcast.value.get(amazonRec)*googleNormsBroadcast.value.get(googleRec))
