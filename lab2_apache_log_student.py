...
parsed_logs.takeSample(False,5)
...
uniqueHostCount = len(uniqueHosts.countByValue().keys())
...
dayHostCount = dayGroupedHosts.map(lambda item: (item[0], set(thing for thing in item[1])))

dailyHosts = (dayHostCount
              .map(lambda pair: (pair[0],len(pair[1]))).cache())
...
daysWithHosts = dailyHosts.sortByKey().map(lambda item: item[0]).collect()
hosts = dailyHosts.sortByKey().map(lambda item: item[1]).collect()
...
avgDailyReqPerHost = (sortedByDay
                      .map(lambda item: (item[0], int((len(item[1]))/len(set(thing for thing in item[1]))))).cache())
...
daysWithAvg = avgDailyReqPerHost.sortByKey().map(lambda item: item[0]).collect()
avgs = avgDailyReqPerHost.sortByKey().map(lambda item: item[1]).collect()
...
daysWithErrors404 = errDateSorted.map(lambda item: item[0]).collect()
errors404ByDay = errDateSorted.map(lambda item: item[1]).collect()
