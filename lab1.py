...
# TODO: Replace <FILL IN> with appropriate code
wordCountsGrouped = wordsGrouped.map(lambda dict_tuple: (dict_tuple[0], sum(list(dict_tuple[1]))))
...
wordCounts = wordPairs.reduceByKey(lambda value_1, value_2: value_1 + value_2)
...
# TODO: Replace <FILL IN> with appropriate code
uniqueWords = len(wordsRDD.countByValue().keys())
print uniqueWords
...
def wordCount(wordListRDD):
    ...
    """
    return wordListRDD.map(lambda x: (x,1)).groupByKey().map(lambda item: (item[0], sum(list(item[1]))))
...
def removePunctuation(text):
    ...
    """
    return ''.join([x for x in text if x.isalpha() or x.isdigit() or x == ' ']).lower().strip()
