...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    """
    return SparseVector(numOHEFeats,sorted([OHEDict[item] for item in rawFeats]),[1.0 for item in rawFeats])
...
sorted(sampleDistinctFeats.collect())
...
def createOneHotDict(inputData):
    ...
    Returns:
        dict: A dictionary where the keys are (featureID, value) tuples and map to values that are
            unique integers.
    """
    return inputData.distinct().zipWithIndex().collectAsMap()
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    Returns:
        SparseVector: A SparseVector of length numOHEFeats with indicies equal to the unique
            ...
            with values equal to 1.0.
    """
    thing1 = set(OHEDict.keys())
    actualRawFeats = [item for item in rawFeats if item in thing1]
...
def getP(x, w, intercept):
    ...
    return 1/float(1+exp(-rawPrediction))
