...
parsedSamplePoints = map(lambda item: parsePoint(item), samplePoints)
...
# TODO: Replace <FILL IN> with appropriate code
parsedData = parsedDataInit.map(lambda item: LabeledPoint(item.label - minYear,item.features))
...
# TODO: Replace <FILL IN> with appropriate code
averageTrainYear = (parsedTrainData.map(lambda label: label.label).mean())
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return np.sqrt(labelsAndPreds.map(lambda pair: squaredError(pair[0],pair[-1])).sum()/float(labelsAndPreds.count()))
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        labelsAndPredsTrain = trainData.map(lambda point: getLabeledPrediction(w,point))
        errorTrain[i] = calcRMSE(labelsAndPredsTrain)
        
        # Calculate the `gradient`.  Make use of the `gradientSummand` function you wrote in (3a).
        # Note that `gradient` should be a `DenseVector` of length `d`.
        gradient = trainData.map(lambda point: gradientSummand(w,point)).sum()
...
labelsAndPreds = parsedValData.map(lambda point: getLabeledPrediction(weightsLR0,point))
...
# TODO: Replace <FILL IN> with appropriate code
labelsAndPreds = parsedValData.map(lambda point: (point.label,firstModel.predict(point.features)))
...
def twoWayInteractions(lp):
    ...
    """
    two_way = np.array([thing[0]*thing[1] for thing in itertools.product(lp.features,lp.features)])
    return LabeledPoint(lp.label,np.hstack((lp.features,two_way)))

print twoWayInteractions(LabeledPoint(0.0, [2, 3]))

# Transform the existing train, validation, and test sets to include two-way interactions.
trainDataInteract = parsedTrainData.map(lambda point: twoWayInteractions(point))
valDataInteract = parsedValData.map(lambda point: twoWayInteractions(point))
testDataInteract = parsedTestData.map(lambda point: twoWayInteractions(point))
