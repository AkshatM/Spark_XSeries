...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    response = (IDandRatingsTuple[0],(len(IDandRatingsTuple[1]),sum(IDandRatingsTuple[1])/float(len(IDandRatingsTuple[1]))))
    return response
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              ...
                              .map(lambda item: (item[1][1][1], item[1][0], item[1][1][0])))
...
def computeError(predictedRDD, actualRDD):
    ...
    predictedReformattedRDD = predictedRDD.map(lambda item: ((item[0],item[1]),item[2]))

    # Transform actualRDD into the tuples of the form ((UserID, MovieID), Rating)
    actualReformattedRDD = actualRDD.map(lambda item: ((item[0],item[1]),item[2]))

    # Compute the squared error for each matching entry (i.e., the same (User ID, Movie ID) in each
    # RDD) in the reformatted RDDs using RDD transformtions - do not use collect()
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD)
                        .map(lambda item: (item[1][0]-item[1][1])**2))
...
validationForPredictRDD = validationRDD.map(lambda item: (item[0],item[1]))
...
myModel = ALS.train(trainingRDD, rank, seed=seed, iterations=iterations,
                      lambda_=regularizationParameter)
testForPredictingRDD = testRDD.map(lambda item: (item[0],item[1]))
...
trainingAvgRating = trainingRDD.map(lambda item: item[2]).sum()/float(trainingRDD.map(lambda item: item[2]).count())
print 'The average rating for movies in the training set is %s' % trainingAvgRating

testForAvgRDD = testRDD.map(lambda item: (item[0],item[1],trainingAvgRating))
...
for ratingsTuple in bestMoviesRDD.take(500):
    print ratingsTuple
...
# TODO: Replace <FILL IN> with appropriate code
movieIDs = [item[1] for item in myRatedMovies]
...
predictedRDD = predictedRatingsRDD.map(lambda ratings: (ratings.product,ratings.rating))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .map(lambda item: (item[1][0],item[0],item[1][1])))
